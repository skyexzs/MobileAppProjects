...
public class HomeActivity extends AppCompatActivity implements FoodAdapter.OnFoodClickListener {
    ...
    private void setFoodAdapter() {
        foodList = foodDb.fetchAllSharedFoodData();
        if (foodList != null) {
    ...
    public void onFoodClick(int position) {
        Intent showIntent = new Intent(this, FoodDetailsActivity.class);
        showIntent.putExtra(Util.FoodKey.FOOD_ID, foodList.get(position).getFood_id());
        startActivityForResult(showIntent, -1);
    }
